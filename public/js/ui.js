...
export const showVideoCallButtons = () => {
  const personalCodeVidBtn = document.getElementById(
    "personal_code_video_button"
  );
  const strangerVidBtn = document.getElementById("stranger_video_button");
  showElement(personalCodeVidBtn);
  showElement(strangerVidBtn);
};
...
export const showNoStrangerDial = () => {
  const infoDial = elements.getInfoDialog(
    "No stranger available",
    "Try later."
  );
  if (infoDial) {
    const dialog = document.getElementById("dialog");
    dialog.appendChild(infoDial);
...
export const updateUIAfterHangUp = (callType) => {
  ...
  if (
    ...
    const chatCallBtns = document.getElementById(
      "finish_chat_button_container"
    );
    hideElement(chatCallBtns);
  }
  const newMsgInput = document.getElementById("new_message");
  hideElement(newMsgInput);
  clearMessenger();
  updateMicButton(false);
  updateCameraButton(false);
  //hide remote video and show placeHolder
  const remoteVid = document.getElementById("remote_video");
